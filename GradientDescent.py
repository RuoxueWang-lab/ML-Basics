...
def batch_gradient_descent(w0, loss_func, grad_loss_func, lr=0.01, 
    ...
    Iteratively updates w by subtracting lr * grad
    ...
    # Arguments
        ...
        grad_func: function calculating the gradient of
            loss_func at w. for vector w, this should return
        ...
        w_stop: stop iterating if w changes by less than
            this (L2 norm)
        max_iter: stop iterating after iterating this
            many times

    # Returns
        ws: a list of the z values at each iteration
...
def stochastic_gradient_descent(w0, loss_func, grad_loss_func, data, batch_size=1, lr=0.01, 
                                loss_stop=1e-4, w_stop=1e-4, max_iter=100):
    """
    Generic mini-batch gradient descent optimisation.
    Iteratively updates w by subtracting lr * grad
    until one or more stopping criteria are met.
